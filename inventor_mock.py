...
class MockOccurrence(MockObject):
    ...
    def __init__(self, name, definition_name, app=None):
        super().__init__(name)
        self.Definition = MockComponentDefinition(definition_name, app=app)
...
class MockAssemblyComponentDefinition(MockObject):
    ...
    def __init__(self, name="AssemblyDef", parent_document=None, app=None):
        super().__init__(name)
        self.Document = parent_document # Añadimos la referencia al documento padre
        # Lista de ocurrencias de componentes simuladas
        self.Occurrences = [
            MockOccurrence("Componente:1", "PiezaA", app=app),
            MockOccurrence("Componente:2", "PiezaB", app=app),
            MockOccurrence("SubEnsamblaje:1", "SubEnsamblajeDef", app=app),
            MockOccurrence("Tornillo:1", "Tornillo_ISO4017", app=app),
            MockOccurrence("Tornillo:2", "Tornillo_ISO4017", app=app),
        ]
        self.BOMQuantity = MockBOMQuantity()
...
def create_new_mock_document(app=None, group_id=None):
    """
    ...
    if group_id == "730":
        ...
        user_props_list.append(MockProperty("Codigo Sizfra", "7001020200006"))
...
class MockDocument(MockObject):
    ...
    def __init__(self, name, property_sets_instance, app=None):
        ...
        self.ComponentDefinition = MockComponentDefinition(name, app=app)
        self.ComponentDefinition.Document = self
        
        # --- INICIO DE LA SOLUCIÓN ---
        # Simulamos la colección 'ComponentDefinitions' que espera la función 'imagen()'.
        # Esta clase interna simula una colección que tiene un método Item().
        class MockComponentDefinitions:
            def Item(self, index):
                # Simplemente devolvemos la única ComponentDefinition que tiene nuestro documento simulado.
                return self.ComponentDefinition
        self.ComponentDefinitions = MockComponentDefinitions()
        self.ComponentDefinitions.ComponentDefinition = self.ComponentDefinition
        # --- FIN DE LA SOLUCIÓN ---
...
class MockApplication:
    ...
    def __init__(self):
        # La instancia aún no está completamente creada, por lo que ActiveDocument es None inicialmente.
        self.ActiveDocument = None
        # Inicia con un documento por defecto
        # Pasamos 'self' para que los sub-objetos puedan referenciar a la aplicación.
        self.ActiveDocument = create_new_mock_document(app=self)

    def CreateNewDocument(self, app=None, group_id=None):
        """Simula la creación de un nuevo documento y lo establece como activo."""
        self.ActiveDocument = create_new_mock_document(app=self, group_id=group_id)
